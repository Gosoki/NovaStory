...
def build_system_script(topic: dict, lang: str = _DEFAULT_LANG) -> str:
    ...
    if _norm(lang) == "zh":
        return (
            ...
            f"{total} 秒。\n"
            "每个镜头需按此顺序包含 4 个字段:【时长】X 秒"
            "(X 是你为该镜分配的具体秒数)、【景别】、【画面描写】、【台词/音效】。\n"
    ...
    return (
        ...
        "各カットには次の4項目をこの順番で含めてください:【秒数】X秒"
        "(Xはそのカットに割り当てた具体的な秒数)、【サイズ】、【画面】、【セリフ・音】。\n"
        "Markdownの番号付きリストで出力し、説明・前置き・まとめは一切加えないでください。\n"
        "出力言語:日本語。"
    )
...
def build_system_revision(topic: dict, lang: str = _DEFAULT_LANG) -> str:
    ...
    if _norm(lang) == "zh":
        return (
            ...
            "每镜按此顺序含【时长】X 秒【景别】【画面描写】【台词/音效】四个字段,Markdown 编号列表。\n"
    ...
    return (
        ...
        "各カットに【秒数】X秒【サイズ】【画面】【セリフ・音】の4項目(この順番)、Markdownの番号付きリスト。\n"
