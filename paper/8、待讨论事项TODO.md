...
- **B2 "终稿后再回炉"是否开放**:第一版 loop 即挂在看稿后(v4 已定);此条改为观察"用户是否抱怨想在更早阶段干预"。
- **B3 时长与疲劳监控**:轮数/净用时分布;E 第 1 轮 5-7 问的作答疲劳,疲劳明显则补充维上限 4→2(config 一行)。
- 伦理审查流程确认(研究科要求)。

## ⚪ 分析层(拿到数据后)

- **A1 「专业质量」怎么测提升**【需要详细讨论】:rubric 按分镜专业维度重写(与引导维度对应,形成"引导什么、提升什么"论证链);judge 3 评+人工双盲校验;或 C 版 vs E 版盲评配对。素材见 reference/06。
- **A3 执行**:探索性同质化对比(方案见上)。
- **C6 token 用量统计**:客观效率指标,API usage 字段落库即可;MVP 后统一加。
